...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
在数字信号处理领域，I路信号和Q路信号通常一起组成复数信号，那么根据欧拉公式，复数形式的接收信号$y_{comp}(t)$可以做如下推导：
\begin{equation}
    \begin{aligned}
        y_{comp}(t) &= y_{in-phase}(t) + j\cdot y_{quad} \\
    ...
    \end{aligned}
\end{equation}
从复数形式的接收信号上，我们可以很直观地看出，最终的接收信号其实是多个路径的复数反射信号以及噪声的叠加，路径$p$对应的复数反射信号的幅值为$\frac{1}{2}\alpha_pg(g-kT_s-T_p)$，相位$\theta_p$为$2\pi f_c T_p$。
