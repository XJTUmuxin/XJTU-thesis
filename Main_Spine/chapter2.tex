...
脉冲超宽带雷达的基带等效表示如图？所示，雷达发射被调制在频率为$f_c$的余弦载波上的高斯脉冲$g(t)$，脉冲信号会以间隔$T_s$周期性地发射，发射信号的数学表示如下：
\begin{equation}
...
\end{equation}
其中$T_p$是由反射路径距离决定的往返飞行时间，$\delta(t)$表示单位冲激函数，那么接收天线接收到的信号$y(t)$就可以被建模成发射信号与信道脉冲响应的卷积再加上噪声$n(t)$，即：
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    \begin{aligned}
        y_{in-phase}(t) &= LPF[y(t)\cdot cos(2\pi f_c(t-kT_s))] \\
...
\begin{equation}
    \begin{aligned}
        y_{quad}(t) &= LPF[y(t)\cdot\sin(2\pi f_c(t-kT_s))] \\
                    &= \frac{1}{2}\sum_{p=1}^{P} \alpha_p g(t-kT_s-T_p) \times \sin(2\pi f_c T_p) + n(t)
    \end{aligned}
\end{equation}
在数字信号处理领域，I路信号和Q路信号通常一起组成复数信号，那么根据欧拉公式，复数形式的接收信号$y_{complex}(t)$可以做如下推导：
\begin{equation}
    \begin{aligned}
        y_{complex}(t) &= y_{in-phase}(t) + j\cdot y_{quad} \\
                    &= \frac{1}{2}\sum_{p=1}^{P}\alpha_p g(t-kT_s-T_p)\times[\cos(2\pi f_c T_p)+j\cdot \sin(2\pi f_c T_p)] + n(t) \\
                    &= \frac{1}{2}\sum_{p=1}^{P}\alpha_p g(t-kT_s-T_p)\cdot e^{j\theta_p} + n(t)
    \end{aligned}
\end{equation}
从复数形式的接收信号上，我们可以很直观地看出，最终的接收信号其实是多个路径的复数反射信号以及噪声的叠加，路径$p$对应的复数反射信号的幅值为$\frac{1}{2}\alpha_pg(g-kT_s-T_p)$，相位$\theta_p$为$2\pi f_c T_p$。至此，我们完成了对脉冲超宽带雷达接收信号的数学推导。
\xsubsection{脉冲超宽带雷达数据结构}{Impulse Radio Ultra-Wideband (IR-UWB) Radar Data Structure}
在上一小节我们对脉冲超宽带雷达从发射信号到接收信号做了严格的数学推导，下面我们对雷达接收信号的最终数据结构以及对应的物理意义进行分析。上一小节提到，雷达会以$T_s$为周期持续发射脉冲信号，我们将从一个脉冲发射开始到该脉冲对应最后一个响应到达结束期间采样到的数据称为一帧(Frame)。在一帧内，时间$t$的概念对应于脉冲信号的飞行时间，因此我们称之为快时间。雷达连续发射脉冲，所有帧按照时间顺序排序，因此，每一帧可以看做是在更慢时间尺度上的采样，我们将其称之为慢时间，通常，快时间的尺度为数十皮秒，而慢时间的尺度为数百微秒。如果将连续接收到的帧按时间顺序排列起来，那么我们可以得到一个二维矩阵，如图?所示。

连续帧沿Y轴（慢时间）排列，在X轴（快时间）上，不同采样点的信号对应着不同时间延迟的反射脉冲响应。由于快时间表示脉冲的往返飞行时间，因此，我们可以将快时间转换成对应的距离，习惯上，我们将一个固定的快时间采样点称为一个距离箱。假如待感知物体处于不同的距离下，那么我们可以通过挑选特定的距离箱来将待感知物体对应的反射信号分离出来，如果物体是动态的，那么我们可以在连续帧上选择特定距离箱内的信号组成时序信号，从该时序信号中我们可以提取出物体的运动信息。一个真实的脉冲超宽带雷达的接收数据二维矩阵如图?所示。

综上所述，脉冲超宽带雷达在一段连续时间内采集的接收信号数据是一个二维的复数矩阵，矩阵的横轴排列着不同的距离箱，纵轴则排列着该距离箱在不同慢时间采样点上的复数信号。当我们感知特定距离上的物体时，我们可以挑选出物体所在的距离箱，持续跟踪该距离箱上复数信号的时序变化，一般来讲，物体的反射信号会跨越多个距离箱，此时需要同时跟踪多个距离箱的复数信号。
\xsection{?}{?}
