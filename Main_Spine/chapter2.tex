...
\begin{figure}[htbp]
    ...
    \label{fig:baseband}
\end{figure}
载波上的高斯脉冲$g(t)$，脉冲信号会以间隔$T_s$周期性地发射，发射信号的数学表示如公式(\ref{equa:trans_signal})所示：
\begin{equation}
    \label{equa:trans_signal}
    x(t) = g(t-kT_s)\cos(2\pi f_c(t-kTs))
\end{equation}
高斯脉冲信号$g(t)$可以表示为公式(\ref{equa:gauss})：
\begin{equation}
    \label{equa:gauss}
    g(t) = V_{tx} exp(-2{\pi}^2{f_B}^2{log}_{10} (e)t^2)
\end{equation}
式中：$f_B$——-10dB带宽；$V_{tx}$——高斯脉冲的最大幅值。雷达发射的信号被环境中的物体反射并由接收天线接收，通常，发射天线和接收天线会被雷达集成到同一位置上。由于感知范围内存在着不同的反射物体，因此信道响应$h(t)$可以表示为具有不同时延和衰减的$P$条路径的总和，如公式(\ref{equa:h(t)})所示：
\begin{equation}
    \label{equa:h(t)}
    h(t) = \sum_{p=1}^{P}\alpha_p \delta(t-T_p)
\end{equation}
式中：$T_p$——反射路径距离决定的往返飞行时间；$\delta(t)$——单位冲激函数；$\alpha_{p}$——该反射路径固有的反射强度系数。那么接收天线接收到的信号$y(t)$就可以被建模成发射信号与信道脉冲响应的卷积再加上噪声$n(t)$，如公式(\ref{equa:y(t)})所示：
\begin{equation}
    \label{equa:y(t)}
    \begin{aligned}
        y(t) &= x(t) * h(t) + n(t) \\
        &= \sum_{p=1}^P \alpha_p g(t - kT_s - T_p) \times \cos\left(2\pi f_c (t - kT_s - T_p)\right) + n(t)
    \end{aligned}
\end{equation}
在接收侧，会对接收信号$y(t)$进行下变频操作，具体操作为将$y(t)$在混频器中与载波余弦及正弦信号相乘，再通过低通滤波器（Low-Pass Filter，LPF），分别得到I路信号（In-phase Signal）和Q路信号（Quadrature Signal），对于I路信号，将$y(t)$与余弦载波$\cos(2\pi f_c(t-kT_S))$相乘后，仅考虑余弦部分，如公式(\ref{equa:m(t)})所示：
\begin{equation}
    \label{equa:m(t)}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
由于$2f_c$频率分量会被低通滤波器滤除，因此只有低频分量$\frac{1}{2}\cos(2\pi f_c T_p)$被留下，因此，在经过下变频和滤波器后的I路信号如公式(\ref{equa:in-phase})所示:
\begin{equation}
    \label{equa:in-phase}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
同理，也可以得到Q路信号，如公式(\ref{equa:quad})所示：
\begin{equation}
    \label{equa:quad}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
在数字信号处理领域，I路信号和Q路信号通常一起组成复数信号，那么根据欧拉公式，可以对复数形式的接收信号$y_\text{complex}(t)$进行推导，如公式(\ref{equa:y_complex})所示：
\begin{equation}
    \label{equa:y_complex}
    \begin{aligned}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{Figures/slow_time_fast_time.png}
    \caption{二维雷达信号矩阵示意图}
...
迁移学习（Transfer Learning）是上述问题的一个有效解决方法。迁移学习的目的是通过迁移不同但相关的源域（Source Domain）中的知识来提高深度学习网络模型在目标域（Target Domain）上的表现，通过这种方式，可以减少神经网络模型对大量目标域数据的依赖性。
关于迁移学习的一般定义为：设源域为 $\mathcal{D}_s = \{X_s, Y_s\}$（数据分布为 $P_s(X,Y)$），目标域为 $\mathcal{D}_t = \{X_t, Y_t\}$（数据分布为 $P_t(X,Y)$），迁移学习的目标是借助源域的知识，优化目标任务的预测函数$f_t:X_t \to Y_t$，其中，源域$\mathcal{D}_s$和目标域$\mathcal{D}_t$的数据分布 $P_s(X,Y)$和$P_t(X,Y)$相近但不完全一致。

迁移学习的关键实现方式大概分为两种，一种为参数迁移（Parameter Transfer），即直接复用源域上模型参数作为目标域上模型参数的初始化并进行微调：
\begin{equation}
    \theta_t = \theta_s +  \Delta\theta
\end{equation}
式中：$\theta_s$——源域上的模型参数；$\theta_t$——目标域上的最终模型参数；$\Delta\theta$——目标域上参数微调量。其中，$\Delta\theta$通过目标域的反向传播优化得到，通常会对源模型的部分层（如深层网络）进行冻结（Freezing），仅训练顶层任务相关参数，避免过度调整底层通用特征。迁移学习的另一种关键实现方式为特征迁移（Feature Transfer），该方法通过特征空间对齐等方法使得源域和目标域的特征在映射后的空间中分布一致，以此实现源域和目标域上特征表示的共享，使模型能够提取跨域的通用特征，从而实现知识的迁移。

一个典型的迁移学习场景如图\ref{fig:transfer_learning}所示，图中展示了迁移学习在跨领域图像分类任务中的应用，源域包含了大量带标注的真实图像，首先在源域上预训练出性能良好的图像分类器，之后将模型权重迁移到卡通风格图像的目标域上，由于源域和目标域极其相关，因此分类器在源域上学习到的知识在目标域上很大程度上同样适用，借助于这种知识复用，仅需将预训练模型在目标域上简单微调就能得到良好的性能表现。
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{Figures/transfer_learning.pdf}
    \caption{迁移学习示意图}
    \label{fig:transfer_learning}
\end{figure}

迁移学习可以分为三种主要类型\cite{zhuang2020comprehensive}：直推式迁移学习（Transductive Transfer Learning）、归纳式迁移学习（Inductive Transfer Learning）和无监督迁移学习（Unsupervised Transfer Learning）。这三种类别迁移学习的差异在于源域和目标域标注数据的可见性，当仅有源域的标注数据可见时，该场景被归类为直推式迁移学习；当目标域中的标注数据也可见时，则被归类为归纳式迁移学习；当源域和目标域的标注数据都不可见时，归类为无监督迁移学习。本文关注归纳式迁移学习，即源域和目标域的标注数据都可见，在这种场景下，迁移学习的训练过程涉及最初在大型数据集上训练基线模型，之后使用较小的目标数据集对其进行微调（Finetune）来达到最佳性能，而并不是直接在相对有限的目标数据集上训练模型，这种训练模式也被称为迁移学习中的“预训练（Pretrain）-微调 （Finetune）”两阶段学习范式。先前实验的经验证据表明，通过这种两阶段学习，迁移学习能够有效地对抗过拟合，加快训练，并尤其降低了在目标域上训练时所需的数据集大小\cite{hendrycks2019using}。
